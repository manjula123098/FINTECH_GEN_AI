...
def plot_comparison_metrics(
    ...
    # 3. NLP Metrics: Entity Extraction
    ax3 = axes[1, 0]
    ax3.bar(x - width/2, rag_entities, width, label='RAG Entities', color='#3498db', alpha=0.8)
    ax3.bar(x + width/2, kg_entities, width, label='KG Entities', color='#e74c3c', alpha=0.8)
    ax3.set_xlabel('Questions')
    ax3.set_ylabel('Entity Count')
    ax3.set_title('NLP Metrics: Entity Extraction & Coverage')
    ax3.set_xticks(x)
    ax3.set_xticklabels(questions)
    ax3.legend()
    ax3.grid(axis='y', alpha=0.3)

    # 4. NLP Quality Score Summary
    ax4 = axes[1, 1]
    avg_metrics = {
        ...
        'Items\n(KG)': np.mean(kg_facts),
        'Quality\n(RAG)': np.mean(rag_quality) if rag_quality else 0,
        'Quality\n(KG)': np.mean(kg_quality) if kg_quality else 0
    }
    ...
    ax4.set_title('NLP Quality Score & Performance Summary')
