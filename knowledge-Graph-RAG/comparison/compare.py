...
from .nlp_metrics import NLPMetricsAnalyzer, display_nlp_metrics
...
async def compare_systems(
    ...
    # Calculate NLP metrics and add to comparison
    nlp_metrics = NLPMetricsAnalyzer.compare_nlp_metrics(
        rag_result['answer'],
        kg_result['answer'],
        rag_result.get('metrics', {}),
        kg_result.get('metrics', {})
    )
    comparison['nlp_metrics'] = nlp_metrics
...
def display_comparison(comparison: Dict[str, Any]) -> None:
    ...
    # Calculate and display NLP metrics
    nlp_metrics = NLPMetricsAnalyzer.compare_nlp_metrics(
        comparison['rag_result']['answer'],
        comparison['kg_result']['answer'],
        comparison['rag_result'].get('metrics', {}),
        comparison['kg_result'].get('metrics', {})
    )
    display_nlp_metrics(nlp_metrics)
